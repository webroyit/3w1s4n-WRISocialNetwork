...
contract SocialNetwork {
    ...
    struct post {
        address postOwner;
        bytes32 parentPost;     // Can be use to get all comments for this post
        bytes32 contentId;
        int40 votes;
        bytes32 categoryId;
    }
    ...
    // Effect amount of voting power based on reputation
    function validateReputationChange(address _sender, bytes32 _categoryId, uint8 _reputationAdded) internal view returns (bool _result){
        ...
        if (_reputation < 2 ) {
            // If reputation is less than 2, the vote count is 1
            _reputationAdded == 1 ? _result = true: _result = false;
        }
        else {
            // If reputation is less than 2, the vote count is 1
            // If reputation is less than 4, the vote count is 2
            // If reputation is less than 8, the vote count is 3
            // If reputation is less than 16, the vote count is 4
